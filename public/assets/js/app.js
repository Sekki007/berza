(function () {
  ...
  function initKpImport() {
    const box = one('[data-kp-import]');
    const form = one('[data-ad-form]');
    if (!box || !form) return;

    const urlInput = one('[data-kp-import-url]', box);
    const ownedCb = one('[data-kp-import-owned]', box);
    const btn = one('[data-kp-import-btn]', box);
    const status = one('[data-kp-import-status]', box);
    const photoBox = one('[data-kp-import-photos]', form);
    ...
    function setAccessories(keys) {
      const list = Array.isArray(keys) ? keys : [];
      form.querySelectorAll('input[name="accessories[]"]').forEach(function (inp) {
        const on = list.indexOf(inp.value) !== -1;
        inp.checked = on;
        const lab = inp.closest('.chip-option');
        if (lab) lab.classList.toggle('is-on', on);
      });
    }

    function renderImportPhotos(paths) {
      if (!photoBox) return;
      photoBox.innerHTML = '';
      const list = Array.isArray(paths) ? paths : [];
      if (!list.length) {
        photoBox.setAttribute('hidden', '');
        return;
      }
      const head = document.createElement('p');
      head.className = 'form-hint';
      head.style.margin = '0 0 8px';
      head.innerHTML = 'Slike sa KP (sačuvaće se pri objavi). <button type="button" class="btn-sm" data-kp-clear-photos>Ukloni uvežene</button>';
      photoBox.appendChild(head);
      list.forEach(function (src, idx) {
        const item = document.createElement('div');
        item.className = 'photo-existing-item';
        item.innerHTML =
          '<img src="' + String(src).replace(/"/g, '&quot;') + '" alt="">' +
          '<input type="hidden" name="kp_import_images[]" value="' + String(src).replace(/"/g, '&quot;') + '">' +
          '<span class="form-hint" style="margin:0;">' + (idx === 0 ? 'Naslovna · ' : '') + 'KP</span>';
        photoBox.appendChild(item);
      });
      photoBox.removeAttribute('hidden');
      const clearBtn = one('[data-kp-clear-photos]', photoBox);
      if (clearBtn) {
    ...
    function applyDraft(draft, images) {
      if (!draft || typeof draft !== 'object') return;

      setRadio('ad_type', draft.ad_type || 'telefon');
      const typeEl = one('[data-form-type]:checked', form);
      if (typeEl) {
        typeEl.dispatchEvent(new Event('change', { bubbles: true }));
      }

      setInput('title', draft.title || '');
      setInput('description', draft.description || '');
      setInput('model', draft.model || '');

      if (draft.price_type === 'negotiable') {
        setRadio('price_type', 'negotiable');
      } else {
        setRadio('price_type', 'fixed');
        setInput('price', draft.price != null ? draft.price : '');
      }
      if (draft.currency === 'rsd') setRadio('currency', 'rsd');
      else setRadio('currency', 'eur');

      const priceInput = one('[data-price-input]', form);
      if (priceInput) priceInput.dispatchEvent(new Event('change', { bubbles: true }));

      setSelect('brand', draft.brand || '');
      setSelect('condition_state', draft.condition_state || 'Polovno');
      setSelect('storage', draft.storage || '');
      if (draft.location) setSelect('location', draft.location);

      if (draft.battery_health != null && draft.battery_health !== '') {
        setInput('battery_health', draft.battery_health);
        const more = one('[data-phone-more]', form);
        const moreBtn = one('[data-phone-more-toggle]', form);
        if (more && more.hasAttribute('hidden')) {
          more.removeAttribute('hidden');
          if (moreBtn) {
            moreBtn.setAttribute('aria-expanded', 'true');
            moreBtn.textContent = 'Manje detalja ▴';
          }
        }
      }

      setAccessories(draft.accessories || []);
      renderImportPhotos(images || []);
    }

    function runImport() {
      const url = (urlInput.value || '').trim();
      if (!url) {
        setStatus('Nalepi link KP oglasa.', 'is-err');
        return;
      }
      if (!ownedCb || !ownedCb.checked) {
        setStatus('Potvrdi da si vlasnik oglasa pre uvoza.', 'is-err');
        return;
      }

      setStatus('Učitavam oglas sa KP…', '');
      btn.disabled = true;

      const body = new FormData();
      body.append('url', url);
      body.append('confirm_owned', '1');
      appendCsrf(body);

      fetch('/api/import-kp.php', {
        method: 'POST',
        body: body,
        credentials: 'same-origin',
        headers: csrfHeaders({ 'Accept': 'application/json' })
        ...
        const data = res.data || {};
        if (!data.ok) {
          setStatus((data.message || 'Uvoz nije uspeo.') + '', 'is-err');
          return;
        }
        applyDraft(data.draft, data.images);
        const n = (data.images && data.images.length) ? data.images.length : 0;
        setStatus((data.message || 'Uvezeno.') + (n ? (' (' + n + ' slika)') : ''), 'is-ok');
        try {
          const title = one('#ad-title', form);
          if (title) title.scrollIntoView({ behavior: 'smooth', block: 'center' });
        } catch (e) {}
      }).catch(function () {
        setStatus('Greška mreže. Pokušaj ponovo.', 'is-err');
      }).finally(function () {
        btn.disabled = false;
      });
    }
